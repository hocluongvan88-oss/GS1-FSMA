...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const handleOnline = () => {
      ...
      setIsOnline(true);
      syncOfflineQueueLocal(); // Use the local function
    };
    const handleOffline = () => {
  ...
  const syncOfflineQueueLocal = async () => {
    if (!session) return;

    const supabaseUrl = import.meta.env.VITE_SUPABASE_URL || 'https://wsjckcqeuiyheomfcaok.supabase.co';
